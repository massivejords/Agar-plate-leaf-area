...
def show_objects(img, contours, hierarchy, label, red=0):
    ...
    if len(ids) > 0:
        ...
        if params.debug is not None:
            cv2.drawContours(ori_img, group, -1, (255-red, 0, red), params.line_thickness)
            for cnt in contours:
                cv2.drawContours(ori_img, cnt, -1, (255-red, 0, red), params.line_thickness)
            if params.debug == 'print':
                print_image(ori_img, os.path.join(params.debug_outdir, f'{label}_debug.png'))
