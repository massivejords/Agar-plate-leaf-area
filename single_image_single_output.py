...
def options():
    parser = argparse.ArgumentParser(
        description="Imaging processing for A. thaliana seedlings with PlantCV")
    parser.add_argument(
        "-i", "--image", help="Input image file.", required=True)
    parser.add_argument(
        "-o", "--outdir", help="Output directory for image files.", required=False, default="./temp")
   # parser.add_argument("-r", "--outfile", help="Result file.", required=True)
    parser.add_argument("-w", "--writeimg", help="Write out images.",
...
def main():
    ...
    thresh = pcv.gaussian_blur(img=thresh, ksize=(
        101, 101), sigma_x=0, sigma_y=None)
    thresh = pcv.threshold.binary(
        gray_img=thresh, threshold=80, max_value=235, object_type="light")
    ...
    b_thresh = pcv.threshold.binary(
        gray_img=b, threshold=t-3, max_value=255, object_type="light")
    ...
    roi_contour, roi_hierarchy = pcv.roi.rectangle(
        img=img, x=100, y=300, h=1300, w=int(width-400))
    ...
    scale_crop = img
    # ROI
    roi_contour_scale, roi_hierarchy_scale = pcv.roi.rectangle(
        img=scale_crop, x=125, y=height-1000, h=780, w=width-540)

    # thresholding
    a_scale = pcv.rgb2gray_lab(rgb_img=scale_crop, channel="b")
    a_scale = pcv.hist_equalization(a_scale)
    a_scale = pcv.gaussian_blur(
        img=a_scale, ksize=(21, 21), sigma_x=0, sigma_y=None)
    a_scale_thresh = pcv.threshold.binary(
        gray_img=a_scale, threshold=253, max_value=255, object_type="light")
    if len(np.unique(a_scale_thresh)) != 1:
        ...
        if len(np.unique(a_scale_thresh)) != 1:
            ...
            if len(np.unique(a_scale_thresh)) != 1:
                ...
                if len(np.unique(a_scale_thresh)) != 1:
                    a_scale_thresh = pcv.erode(
                        gray_img=a_scale_thresh, ksize=9, i=1)
                    if len(np.unique(a_scale_thresh)) != 1:
                        a_scale_thresh = pcv.fill(
                            bin_img=a_scale_thresh, size=500)
                        if len(np.unique(a_scale_thresh)) != 1:
                            a_scale_thresh = pcv.dilate(
                                gray_img=a_scale_thresh, ksize=9, i=1)
    id_scale, obj_hierarchy = pcv.find_objects(
        img=scale_crop, mask=a_scale_thresh)
    ...
    if scale_pixels == 0:
        a_scale_thresh = pcv.threshold.binary(
            gray_img=a_scale, threshold=249, max_value=255, object_type="light")
        if len(np.unique(a_scale_thresh)) != 1:
            a_scale_thresh = pcv.closing(gray_img=a_scale_thresh)
            if len(np.unique(a_scale_thresh)) != 1:
                a_scale_thresh = pcv.fill(bin_img=a_scale_thresh, size=11000)
    ...
    if scale_pixels == 0:
        ...
        h_scale = pcv.gaussian_blur(
            img=h_scale, ksize=(31, 31), sigma_x=0, sigma_y=None)
        h_scale_thresh = pcv.threshold.binary(
            gray_img=h_scale, threshold=251, max_value=255, object_type="light")
        if len(np.unique(h_scale_thresh)) != 1:
            ...
            if len(np.unique(h_scale_thresh)) != 1:
                ...
                if len(np.unique(h_scale_thresh)) != 1:
                    ...
                    if len(np.unique(h_scale_thresh)) != 1:
                        h_scale_thresh = pcv.erode(
                            gray_img=h_scale_thresh, ksize=9, i=1)
                        if len(np.unique(h_scale_thresh)) != 1:
                            h_scale_thresh = pcv.fill(
                                bin_img=h_scale_thresh, size=500)
                            if len(np.unique(h_scale_thresh)) != 1:
                                h_scale_thresh = pcv.dilate(
                                    gray_img=h_scale_thresh, ksize=9, i=1)
        id_scale_2, obj_hierarchy_2 = pcv.find_objects(
            img=scale_crop, mask=h_scale_thresh)
        roi_scale, scale_hierarchy, scale_mask, scale_area = pcv.roi_objects(img=scale_crop,
                                                                             roi_contour=roi_contour_scale, roi_hierarchy=roi_hierarchy_scale,
    ...
    if scale_pixels == 0:
        ...
        h_scale = pcv.gaussian_blur(
            img=h_scale, ksize=(31, 31), sigma_x=0, sigma_y=None)
        h_scale_thresh = pcv.threshold.binary(
            gray_img=h_scale, threshold=235, max_value=255, object_type="light")
        if len(np.unique(h_scale_thresh)) != 1:
            h_scale_thresh = pcv.closing(gray_img=h_scale_thresh)
            if len(np.unique(h_scale_thresh)) != 1:
                h_scale_thresh = pcv.fill(bin_img=h_scale_thresh, size=12000)
    ...
    if scale_pixels == 0:
        ...
        h_scale = pcv.gaussian_blur(
            img=h_scale, ksize=(31, 31), sigma_x=0, sigma_y=None)
        h_scale_thresh = pcv.threshold.binary(
            gray_img=h_scale, threshold=245, max_value=255, object_type="light")
        if len(np.unique(h_scale_thresh)) != 1:
            h_scale_thresh = pcv.closing(gray_img=h_scale_thresh)
            if len(np.unique(h_scale_thresh)) != 1:
                h_scale_thresh = pcv.fill(bin_img=h_scale_thresh, size=12000)
    ...
    if scale_pixels == 0:
        ...
        h_scale = pcv.gaussian_blur(
            img=h_scale, ksize=(31, 31), sigma_x=0, sigma_y=None)
        h_scale_thresh = pcv.threshold.binary(
            gray_img=h_scale, threshold=10, max_value=255, object_type="dark")
        if len(np.unique(h_scale_thresh)) != 1:
            h_scale_thresh = pcv.closing(gray_img=h_scale_thresh)
            if len(np.unique(h_scale_thresh)) != 1:
                h_scale_thresh = pcv.fill(bin_img=h_scale_thresh, size=12000)
    ...
    for i in range(0, 6):
        ...
        if clusters_i[i][0] != None:
            id_objects, obj_hierarchy = pcv.find_objects(
                img=imgs[num_plants], mask=masks[num_plants])
            obj, mask1 = pcv.object_composition(
                img=imgs[num_plants], contours=id_objects, hierarchy=obj_hierarchy)
            m = cv2.moments(obj)
            ...
            entry = {'position': pos, 'area': float(
                area/pixels_per_mm2), 'suspicious': leaf_error}
            plant_areas[f'plant_{pos}'] = entry

        else:
            entry = {'position': pos, 'area': 0, 'suspicious': None}
            plant_areas[f'plant_{pos}'] = entry
